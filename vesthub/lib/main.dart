// lib/main.dart — v0.0.5 Firebase Auth
...
class _SimulatorsTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Consumer<AppState>(
      builder: (_, state, __) {
        final sims = [
          _S('SISU', 'Universidades Federais', Icons.school_outlined, AppTheme.accentGreen,
              SisuScreen(courses: state.sisuCourses)),
          _S('SSA · UPE', 'Selecao Seriada UPE', Icons.trending_up_outlined, const Color(0xFFF87171),
              SsaScreen(courses: state.ssaCourses)),
          _S('PAS · UnB', 'Avaliacao Seriada UnB', Icons.timeline_outlined, const Color(0xFF8B5CF6),
              PasScreen(courses: state.pasCourses)),
          _S('FUVEST · USP', 'Vestibular da USP', Icons.star_outline, const Color(0xFF4ADE80),
              FuvestScreen(courses: state.fuvestCourses)),
          _S('UNICAMP', 'COMVEST', Icons.bar_chart_outlined, AppTheme.accent,
              UnicampScreen(courses: state.unicampCourses)),
          _S('UNESP · VUNESP', 'Vestibular UNESP', Icons.emoji_events_outlined, AppTheme.accentAmber,
              UnespScreen(courses: state.unespCourses)),
        ];
              ...
              children: [
                ...
                Expanded(
                  child: ListView.separated(
                    ...
                    itemBuilder: (_, i) => _SimTile(s: sims[i]),
                  ),
                ),
              ],
            ),
...
class _SimTile extends StatelessWidget {
  final _S s;
  const _SimTile({required this.s});
  @override
  Widget build(BuildContext context) {
    return GestureDetector(
