...
class _FuvestScreenState extends State<FuvestScreen> with SingleTickerProviderStateMixin {
  ...
  void _simulate(ExamFormulas formulas) {
    ...
    setState(() => _result = CalcService.instance.simularFuvest(
          scores: _scores,
  ...
  Widget build(BuildContext context) {
    ...
    final fuv = formulas.fuvest;
    final notaFinal = CalcService.instance.calcFuvestNota(_scores, formulas);
    final qMax = fuv.questaoMax;
        ...
        child: Column(children: [
          ...
          GlassBox(margin: const EdgeInsets.symmetric(horizontal: 18), padding: const EdgeInsets.all(14), child: Row(children: [
            ...
            Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
              Text('F1: ${_scores.fase1Total.toStringAsFixed(0)}/${fuv.fase1MaxTotal.toInt()}', style: AppTheme.bodySmall),
              Text('F2: ${_scores.fase2Total.toStringAsFixed(1)}/${fuv.fase2Max.toInt()}', style: AppTheme.bodySmall),
            ]),
          ])),
          ...
          Expanded(child: TabBarView(controller: _tabController, children: [
            SingleChildScrollView(padding: const EdgeInsets.all(18), child: GlassBox(padding: const EdgeInsets.all(16), child: Column(children: [
              ScoreInputField(label: 'Portugues', hint: '0', min: 0, max: qMax, value: _scores.portugues, suffix: '/${qMax.toInt()}',
                  onChanged: (v) => setState(() => _scores = _scores.copyWith(portugues: v))),
              const SizedBox(height: 14),
              ScoreInputField(label: 'Matematica', hint: '0', min: 0, max: qMax, value: _scores.matematica, suffix: '/${qMax.toInt()}',
                  onChanged: (v) => setState(() => _scores = _scores.copyWith(matematica: v))),
              const SizedBox(height: 14),
              ScoreInputField(label: 'Ciencias', hint: '0', min: 0, max: qMax, value: _scores.ciencias, suffix: '/${qMax.toInt()}',
                  onChanged: (v) => setState(() => _scores = _scores.copyWith(ciencias: v))),
              const SizedBox(height: 14),
              ScoreInputField(label: 'Historia e Geografia', hint: '0', min: 0, max: qMax, value: _scores.historia, suffix: '/${qMax.toInt()}',
            ...
            SingleChildScrollView(padding: const EdgeInsets.all(18), child: Column(children: [
              GlassBox(padding: const EdgeInsets.all(16), child: Column(children: [
                ScoreSlider(label: 'Redacao (0-${fuv.redacaoMax.toInt()})', min: 0, max: fuv.redacaoMax, value: _scores.redacao,
                    activeColor: AppTheme.accentGreen, onChanged: (v) => setState(() => _scores = _scores.copyWith(redacao: v))),
                const Divider(height: 20),
                ScoreSlider(label: 'Discursivas (0-${fuv.fase2Max.toInt()})', min: 0, max: fuv.fase2Max, value: _scores.fase2Total,
