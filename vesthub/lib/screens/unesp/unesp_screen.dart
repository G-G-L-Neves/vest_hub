      ...
      body: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          GlassBox(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text('/ 198 pontos', style: AppTheme.bodySmall),
            const SizedBox(height: 8),
            GlassProgressBar(value: total / 198, aprovado: total >= 110 ? true : false),
          ])),
          const SizedBox(height: 14),
          GlassBox(padding: const EdgeInsets.all(16), child: Column(children: [
            ScoreSlider(label: 'Fase 1 (0-72)', min: 0, max: 72, value: _scores.fase1,
                activeColor: AppTheme.accent, onChanged: (v) => setState(() => _scores = _scores.copyWith(fase1: v))),
            const Divider(height: 20),
            ScoreSlider(label: 'Redacao (0-30)', min: 0, max: 30, value: _scores.redacao,
                activeColor: AppTheme.accentGreen, onChanged: (v) => setState(() => _scores = _scores.copyWith(redacao: v))),
            const Divider(height: 20),
            ScoreSlider(label: 'Discursiva (0-96)', min: 0, max: 96, value: _scores.fase2Disc,
                activeColor: AppTheme.accentAmber, onChanged: (v) => setState(() => _scores = _scores.copyWith(fase2Disc: v))),
          ])),
          const SizedBox(height: 16),
          CourseSelector<Course>(label: 'Curso (UNESP)', courses: widget.courses, selected: _selectedCourse,
